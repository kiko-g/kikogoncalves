      ...
      <TabGroup selectedIndex={tabs.indexOf(selectedTab)} onChange={(index) => setSelectedTab(tabs[index])}>
        <TabList className="mb-4 flex gap-4">
          {tabs.map((tab) => (
            <Tab
              key={tab}
              className="rounded bg-black/30 px-3 py-1 text-sm/6 font-semibold text-white transition-all data-[selected]:bg-black/70 data-[hover]:opacity-80 data-[selected]:data-[hover]:opacity-100 dark:data-[hover]:bg-white/5 dark:data-[selected]:bg-white/10 dark:data-[selected]:data-[hover]:bg-white/10"
            >
              {tab}
            </Tab>
          ))}
